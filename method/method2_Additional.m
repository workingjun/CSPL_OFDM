function [e_rx, e_rx_pdf, e_rx_multi_pdf, y_rx, y_rx_pdf, y_rx_multi_pdf, L_sol_erx] = method2_Additional(params)
    N = params.N;
    GP = params.GP;
    L = params.L;
    ...
    mean_e = params.J(L+1) ./ (GP - (1:GP) + 1); 
    sigma_e = mean_e.^2 / N_OFDM_symbols;

    e_rx_pdf = (2 * pi * sigma_e).^(-0.5) .* exp(-0.5 .* (abs(e_rx - mean_e).^2 ./ sigma_e));
